...
async def check_membership(uid: int) -> tuple[bool, bool, str]:
    """Check if user is member of both required chats using GetParticipantRequest."""
    channel_ok = False
    group_ok = False
    error_msg = ""
    try:
...
async def send_realtime_hit(user_id, result, hit_type):
    # 1. Send full details to hits channel
    ...
    # 2. Send clean log to logs channel
    await send_hit_log(user_id, result, hit_type)
    # 3. Send to user (as before)
...
async def send_insufficient_log(user_id, result):
    ...
    # No log sent to channel (optional)
...
async def add_premium_command(event):
    ...
    # Send purchase log
    await send_purchase_log(int(new_id), plan="Premium", days=1, limit=LIMITS["premium"])
...
# ----- Callback handlers (with membership checks) -----
