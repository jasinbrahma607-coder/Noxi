# bot.py – NOXI Bot (Full Source, Branded)
...
JOIN_GROUP_ID = int(os.getenv("JOIN_GROUP_ID", "-1004381920430"))  # Replace with your group ID
...
HIT_BUTTON = [[Button.url(bs("NOXI"), "https://t.me/NOXI_Bot")]]  # Replace with your bot username
...
def is_site_error(text):
    ...
    if lower == 'na':
        return True
    # These are NOT errors – they mean the site is alive but too expensive
...
async def test_site(site, proxy_data=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url) as resp:
            if resp.status != 200:
                return {'site': site, 'status': 'dead', 'price': '-', 'response': f'HTTP_{resp.status}'}
            try:
                raw = await resp.json(content_type=None)
            except:
                return {'site': site, 'status': 'dead', 'price': '-', 'response': 'Invalid JSON'}
        rm = raw.get('Response', '')
        price = raw.get('Price', '-')
        if price and price != '-':
            price = f"${price}"
