# bot.py – NOXI Bot (Fast Local Checker)
...
# UPDATED: Default hit channel changed to your provided ID
...
# Override get_bin_info with fast local + HTTP fallback
        ...
        async with _BIN_SEM:
            async with s.get(f'https://bins.antipublic.cc/bins/{bin6}') as r:
                if r.status == 200:
                    ...
                    _BIN_HTTP_CACHE[bin6] = (
                        result["brand"], result["type"], result["level"],
                        result["bank"], result["country"], result["flag"]
                    )
...
def _proxy_to_checker_format(proxy_data):
    """Convert proxy dict to string like http://user:pass@host:port"""
    if not proxy_data:
        return None
...
async def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    """Call local checker API with POST /check"""
    try:
        ...
        if status == "CHARGED":
            ...
            status_out = "SiteError"  # treat as site error for retry
...
|   {PE} <code>/sh</code> ━ <b>{bs('Single CC')}</b>
|   {PE} <code>/msh</code> ━ <b>{bs('Mass CC')}</b>
...
<b>━━━━━━━━━━━━━━━━━</b>
{sl}"""
        kb = [[pbtn(bs("Plans"), data="show_plans"), pbtn(bs("Support"), url="https://t.me/SUPERGREMLIN01")],
...
async def show_plans(event):
    ...
    await styled_reply(event, plans_text, buttons=[[pbtn(bs("Upgrade"), url="https://t.me/SUPERGREMLIN01")]], emoji_ids=[CE["fire"], CE["fire"], CE["crown"]])
...
async def send_final_file(uid, charged, approved, declined, errors, total, hits=None, target_chat=None):
    hits = hits or []
    fn = f"noxi_{uid}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
async def mass_check_cmd(event):
    ...
    cmd_text = re.sub(r'^[/.]msh\s*', '', event.raw_text, flags=re.IGNORECASE).strip()
