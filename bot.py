...
from config import (
    ...
    LIMITS, MASS_WORKERS,
    REQUIRED_CHANNEL_ID, HITS_CHANNEL_ID, LOGS_CHANNEL_ID,
...
async def is_user_member(uid: int) -> bool:
    """Check if user is a member of REQUIRED_CHANNEL_ID."""
    try:
...
async def forward_hit_to_channel(result: dict, user_id: int, hit_type: str):
    ...
    # Build a clean message without user-specific inline buttons
    msg = (
        f"<b>{header}</b>\n"
        f"<b>{SEP}</b>\n"
...
# ----- START of event handlers (with membership checks) -----
...
async def start(event):
    ...
    if not await is_user_member(uid):
        await event.reply(pe(
            ...
            f"👉 <a href='{CHANNEL_INVITE_LINK}'>Join Channel</a>\n"
            f"Then restart the bot with /start."
...
async def single_check(event):
    uid = event.sender_id
    if not await is_user_member(uid):
        await event.reply(pe(f"🔒 Join required: {CHANNEL_INVITE_LINK}"), parse_mode='html')
        return
...
async def mass_check_cmd(event):
    ...
    if msg_id:
        ...
        await send_log(f"User {uid} started mass check with {len(cards)} cards")

# ----- Admin commands (no membership check required for admins, but we keep it anyway) -----
...
async def add_proxy_command(event):
    ...
    await send_log(f"Admin {uid} added {len(added)} proxies")
...
async def clear_all_proxies(event):
    ...
    await send_log(f"Admin {uid} cleared proxy pool ({len(curr)} proxies)")
...
async def site_command(event):
    ...
    await send_log(f"User {uid} tested sites: {len(alive)} alive, {len(dead)} removed")
...
async def remove_site_command(event):
    ...
    await send_log(f"Admin {uid} removed site: {site}")
...
async def proxy_command(event):
    ...
    await send_log(f"User {uid} tested proxies: {len(alive)} alive, {len(dead)} dead")
...
async def setadmin_command(event):
    ...
    if action in ("add", "rm", "remove") and len(parts) >= 3:
        ...
        if action == "add":
            ...
            await send_log(f"Admin {uid} added admin {target}")
        else:
            ...
            await send_log(f"Admin {uid} removed admin {target}")
    else:
        await event.reply(pe(
            f"<b>👑 Admin Management</b>\n"
            f"<b>{SEP}</b>\n"
...
async def add_premium_command(event):
    ...
    await send_log(f"Admin {event.sender_id} added premium user {new_id}")
...
async def remove_premium_command(event):
    ...
    await send_log(f"Admin {event.sender_id} removed premium user {rm_id}")
...
async def add_site_command(event):
    ...
    await send_log(f"Admin {event.sender_id} added site: {new_site}")
...
async def broadcast_command(event):
    ...
    await send_log(f"Admin {uid} broadcasted to {len(all_targets)} users")

# ----- Callback handlers (with membership checks where needed) -----
...
# ----- Admin callback handlers (no membership required) -----
...
async def cb_admin_clear_proxy(event):
    ...
    await nav_edit(event.chat_id, event.message_id, _admin_panel_text(), rows_admin())
    await send_log(f"Admin {event.sender_id} cleared proxy pool via button")
...
async def cb_start_check(event):
    ...
    if msg_id:
        asyncio.create_task(run_mass_check(uid, cards, msg_id))
        await send_log(f"User {uid} started mass check from file with {len(cards)} cards")
