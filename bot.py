...
async def check_membership(uid: int) -> tuple[bool, bool, str]:
    """
    Returns (channel_ok, group_ok, error_message)
    """
    channel_ok = False
    group_ok = False
    error_msg = ""
    ...
    try:
        ...
        error_msg = f"Channel check failed: {str(e)[:50]}"
    ...
    try:
        ...
        if error_msg:
            error_msg += " | "
        error_msg += f"Group check failed: {str(e)[:50]}"

    return channel_ok, group_ok, error_msg

async def is_user_member(uid: int) -> bool:
    """Check if user is member of BOTH required chats."""
...
# ----- Verification UI (with detailed status) -----
...
async def start(event):
    ...
    if not channel_ok or not group_ok:
        ...
        await send_log(f"Membership check failed for {uid}: {error}")
...
# ----- Debug command for admins to check membership -----
...
# ----- All other commands (with membership check) -----
...
# ----- Admin commands (no membership check required for admins) -----
...
def _register_commands():
    user_cmds = [
        ...
        {"command": "checkme",        "description": "🔍 Check your membership status (admin only)"},
    ]
